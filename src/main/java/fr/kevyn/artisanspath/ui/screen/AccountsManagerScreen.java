...
public class AccountsManagerScreen extends AbstractMenuScreen<BaseMenu> {
  ...
  public AccountsManagerScreen(BaseMenu menu, Inventory playerInventory) {
    super(
        menu,
        playerInventory,
        Component.translatable("gui.artisanspath.accounts_management.title"));
  ...
  private void addWidgets() {
    this.accounts =
        new ArrayList<>(this.player.getData(ArtisansDataAttachments.ACCOUNTS).values());
    ...
    if (accounts.size() < AccountUtils.MAX_ACCOUNT) {
      SilentButton createAccountButton = new SilentButton(
          ...
          100,
          buttonHeight,
          Component.translatable("gui.artisanspath.create_account").withStyle(ChatFormatting.GREEN),
